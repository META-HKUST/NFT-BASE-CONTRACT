...
func _countOf(ctx contractapi.TransactionContextInterface, owner string) int {
	iterator, err := ctx.GetStub().GetStateByPartialCompositeKey(countPrefix, []string{owner})
	if err != nil {
		panic("Error creating asset chaincode:" + err.Error())
	}
...
func (c *TokenERC721Contract) BaseURI(ctx contractapi.TransactionContextInterface) string {
	baseURIBytes, err := ctx.GetStub().GetState(baseURIPrefix)
	if err != nil {
		return "failed to GetState of baseURI"
	}
...
func _getUserHash(ctx contractapi.TransactionContextInterface, userId string) string {
	hash := sha256.New()
	hash.Write([]byte(userId))
	userHash := hex.EncodeToString(hash.Sum(nil))
	uCount := _countOf(ctx, userId)
	userCount := strconv.Itoa(uCount)
	id := userHash +"-" + userCount
	return id
}
...
func _mint(ctx contractapi.TransactionContextInterface) (*Nft, error) {
	...
	tokenId := _getUserHash(ctx, minter)
	...
	//count
	countKey, err := ctx.GetStub().CreateCompositeKey(countPrefix, []string{minter, tokenId})
	if err != nil {
		return nil, fmt.Errorf("failed to CreateCompositeKey to countKey: %v", err)
	}

	err = ctx.GetStub().PutState(countKey, []byte{'\u0000'})
	if err != nil {
		return nil, fmt.Errorf("failed to PutState countKey %s: %v", nftBytes, err)
	}
...
func _ntimesmint(ctx contractapi.TransactionContextInterface, Id string) (*Nft, error) {
...
func _burn(ctx contractapi.TransactionContextInterface, tokenId string) (bool, error) {
