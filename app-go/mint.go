...
func main() {
	...
	fmt.Println("SetChaincodeOwner:")
	SetChaincodeOwner(contract)
...
func TokenURI(contract *client.Contract) {
	...
	ownerer, err := contract.EvaluateTransaction("TokenURI", "1")
	if err != nil {
...
func SetChaincodeOwner(contract *client.Contract) {
	...
	chainCodeOwner, err := contract.SubmitTransaction("SetChainCodeOwner","x509::CN=user1,OU=client,O=Hyperledger,ST=North Carolina,C=US::CN=ca.org1.example.com,O=org1.example.com,L=Durham,ST=North Carolina,C=US")
	if err != nil {
		panic(fmt.Errorf("failed to SubmitTransaction: %w", err))
	}
...
func PrivateBurn(contract *client.Contract) {
	...
	_, err := contract.SubmitTransaction("PrivateBurn", "2")
	if err != nil {
